...
const FormContact = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (code == 200) {
        message.success('Bạn đã đăng ký tài khoản thành công, nhân viên sẽ liên hệ cho bạn trong thời gian')
      }
    } catch (error) {
    ...
    >
      <Row gutter={[0, 24]}>
        ...
        <Col span={24}>
          <Form.Item name='name'
            rules={[
              { required: true, message: texts?.RSPASS_EMPTY_NAME}
            ]}
          >
          ...
          <Row gutter={[16, 0]}>
            <Col xs={24} md={24} lg={12}>
              ...
              >
                <Input size={size} placeholder={`${texts?.LABEL_PHONE_NUMBER} (*)`} />
              </Form.Item>
            </Col>
            ...
            <Col xs={24} md={24} lg={12}>
              <Form.Item name='source'
                rules={[
                  { required: true, message: texts?.ADD_SPA_ENTER_NAME}
                ]}
              >
